...
public interface ApiInterface {
    ...
    @Headers(key)
    @GET("gym/api/personas/{id}") //revisar las url a las q le pegas la httprequest
    Call<Persona> getUserForId(@Path("id") String id);

    //Esta la utilizas para crear un usuario, neecesitas mandarle el modelo completo
    @Headers(key)
    @POST("gym/api/personas/") //revisar las url a las q le pegas la httprequest
    Call<Boolean> createUser(@Body Persona persona);

    //actualizacion de usuario, mandar modelo con el id
    @Headers(key)
    @PUT("gym/api/personas/{id}") //revisar las url a las q le pegas la httprequest
    Call<String> updateUser(@Body Persona persona);
